...
// --- Plugin system: load .ts/.js plugins from ~/.kbserve/plugins/ ---
const PLUGINS_DIR = join(process.env.EVOLVE_HOME, "plugins")
const plugins: Array<{ name: string; enabled: boolean; hooks: any }> = []
...
function loadPlugins(): void {
  try {
    ...
    for (const entry of readdirSync(PLUGINS_DIR, { withFileTypes: true })) {
      ...
      // Plugin loading is async — stash names for now
      plugins.push({ name: entry.name.replace(/\.(ts|js)$/, ""), enabled: true, hooks: {} })
    }
  } catch {}
}
...
const server = createServer(async (req, res) => {
  ...
  // Health
  if (url === "/health") { json(res, { ok: true, pid: process.pid, plugins: plugins.length }); return }
  ...
  if (url.startsWith("/admin/")) {
    try {
      ...
      switch (action) {
        ...
        // Plugins
        case "plugins/list": result = { plugins: plugins.map((p) => ({ name: p.name, enabled: p.enabled })) }; break
        case "plugins/toggle": { const p = plugins.find((pl) => pl.name === body.name); if (p) { p.enabled = !p.enabled; result = { ok: true, name: p.name, enabled: p.enabled } } else result = { error: "not found" }; break }
      }
      json(res, result)
    } catch (e) { json(res, { error: (e as Error).message }, 400) }
    return
...
server.listen(PORT, () => {
  ...
  console.log(`  plugins dir: ${PLUGINS_DIR} (${plugins.length} loaded)`)
})