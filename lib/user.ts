...
export function userAdd(keyword: string, content: string) {
  ...
  const st = stamp()
  const info = db.query("INSERT INTO user_profile (uuid, origin, keyword, content, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)").run(st.uuid, st.origin, keyword, content, ts, ts)
  return { keyword, content, id: Number(info.lastInsertRowid), uuid: st.uuid }
}

export function userList() {
  return getDb().query("SELECT keyword, content, created_at FROM user_profile WHERE deleted = 0 ORDER BY created_at DESC").all()
}

export function userRemove(keyword: string) {
  const res = getDb().query("UPDATE user_profile SET deleted = 1, updated_at = ? WHERE keyword = ? AND deleted = 0").run(now(), keyword)
